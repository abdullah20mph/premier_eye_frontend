...
export default function AICallsPage({ leads, onSelectLead }: Props) {
  ...
  // ✅ Use the real React.useEffect hook
  useEffect(() => {
    let cancelled = false;
    ...
    async function load() {
      try {
        ...
        console.error("Failed to load AI calls feed", err);
  ...
  // Flatten calls (backend first, fallback to mock data)
  const allCalls = useMemo(() => {
    if (callsFromBackend.length) {
      // Backend already returns flattened calls
      return callsFromBackend as unknown as CallItem[];
    }

    // Fallback: derive from leads.callAttempts
  ...
  const getOutcomeStyle = (outcome: string | null | undefined) => {
    switch (outcome) {
      case 'answered':
    ...
    <div className="max-w-5xl mx-auto h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4 shrink-0">
        <div>
          ...
          {loading && (
            <p className="text-xs text-gray-400 mt-1">Loading calls…</p>
          )}
          {error && (
        ...
        <div className="flex gap-2">
          ...
          <select
            ...
            value={filter}
            onChange={(e) => {
              setFilter(e.target.value as 'all' | 'answered' | 'voicemail');
              setCurrentPage(1);
            }}
          >
                ...
                {calls.map((call) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                      ...
                      <div className="col-span-1">
                        <div
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase w-fit flex items-center gap-1 ${getOutcomeStyle(
                            call.outcome
                          )}`}
                        >
