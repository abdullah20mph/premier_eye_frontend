                                    ...
                                    <div key={c.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                                        <div className="flex justify-between items-start mb-2">
                                            <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full tracking-wide ${
                                                c.outcome === 'answered' || c.outcome === 'booked' ? 'bg-green-100 text-green-700' : 
                                                c.outcome === 'voicemail' ? 'bg-orange-100 text-orange-700' : 
                                                'bg-gray-200 text-gray-600'
                                            }`}>
                                                {c.outcome.replace('_', ' ')}
                                            </span>
                                            <span className="text-[10px] text-gray-400">{format(new Date(c.ts), "MMM d, h:mm a")}</span>
                                        </div>
                                        ...
                                        {c.recordingUrl && (
                                            <div className="mb-3 bg-white rounded-md border border-gray-200 p-2 flex items-center gap-3 cursor-pointer hover:bg-gray-50 transition group shadow-sm">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    {c.durationSec && <div className="text-[10px] text-gray-400">{Math.floor(c.durationSec / 60)}:{(c.durationSec % 60).toString().padStart(2, '0')} duration</div>}
                                                </div>
                                            </div>
                                        )}
