...
import { Search, ChevronRight, Filter, Calendar, DollarSign, MapPin } from 'lucide-react';
...
export default function LeadTable({ leads, onSelect }: Props) {
  ...
  const sourceOptions = Array.from(new Set(leads.map((l) => l.source || "Unknown").filter(Boolean)));
        ...
        <table className="min-w-full">
          <thead className="bg-gray-50/50 border-b border-gray-100">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider hidden xl:table-cell">Activity</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider hidden 2xl:table-cell">Notes</th>
            ...
            {filtered.map((l) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusStyles[l.status] || "bg-gray-100 text-gray-600"}`}>
                    {l.status}
                  </span>
                </td>
                ...
                <td className="px-6 py-4 text-sm text-gray-500 hidden 2xl:table-cell max-w-xs truncate">
                   {l.notes || <span className="text-gray-300 italic">No notes</span>}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-right">
                   {l.saleAmount ? (
                      <span className="font-bold text-brand-black">
                        ${l.saleAmount.toLocaleString()}
                      </span>
                   ) : (
