...
export default function LeadsBoard({ leads, onUpdateStatus, onSelectLead }: Props) {
  ...
  // Helper to get next logical status for "Quick Move" button
        ...
        <div className="flex gap-4 min-w-[1200px] h-full">
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              ...
              >
                {/* Column Header */}
                <div className={`p-3 border-t-4 ${col.color} bg-white rounded-t-xl border-b border-gray-100 mb-2`}>
                     ...
                     >
                        ...
                        <div className="flex items-center gap-3 overflow-hidden">
                           ...
                           <div className="min-w-0">
                              <div className="font-bold text-sm text-brand-black truncate">{lead.name}</div>
                              
                              {/* Location Badge (New) */}
                              {lead.location && (
                                <div className="flex items-center gap-1 mt-0.5">
                                    <MapPin className="w-3 h-3 text-gray-400" />
                                    <span className="text-[10px] font-medium text-gray-500 truncate">{lead.location}</span>
                                </div>
                              )}
                              
                              {/* Minimal Status text if special case */}
                              {lead.status === 'No Show' && (
                                <div className="text-[10px] font-bold text-red-600 mt-1">NO SHOW</div>
                              )}
                              {lead.status === 'Needs VA Follow-Up' && (
                                <div className="text-[10px] font-bold text-red-500 mt-1">NEEDS ACTION</div>
                              )}
                           </div>
                        </div>

                        {/* Alert Icon (Right Side) */}
                        {(lead.status === 'Needs VA Follow-Up' || lead.status === 'No Show') && (
                           <div className="text-red-500 animate-pulse shrink-0 ml-2">
                              <AlertCircle className="w-4 h-4" />
                           </div>
                        )}
                        
                        {/* Quick Move Arrow (Hover only) */}
                         {getNextStatus(lead.status) && !((lead.status === 'Needs VA Follow-Up' || lead.status === 'No Show')) && (
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
