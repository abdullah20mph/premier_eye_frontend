...
import { LayoutDashboard, Users, Calendar, BarChart3, Settings, Phone, MessageCircle, ChevronLeft, ChevronRight, Menu, X } from 'lucide-react';
...
export default function App() {
  ...
  const alertLeads = useMemo(() => {
    return leads.filter(l => l.status === "Needs VA Follow-Up");
  }, [leads]);
    ...
    <div className="flex min-h-screen bg-brand-bg font-sans text-brand-text overflow-hidden">
      ...
      >
        ...
        <div className="p-6 flex flex-col h-full">
          ...
          {/* Mobile Sidebar Header (inside drawer) */}
...
function NavItem({ icon: Icon, label, active, onClick, collapsed }: { icon: any, label: string, active?: boolean, onClick: () => void, collapsed: boolean }) {
  return (
    <button 
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition font-medium text-left ${active ? 'bg-gray-800 text-brand-blue' : 'text-gray-400 hover:text-white hover:bg-gray-900'} ${collapsed ? 'md:justify-center' : ''}`}
    >
      <Icon className="w-5 h-5 shrink-0" />
      {/* Hide label only on desktop collapsed mode, show on mobile drawer */}
